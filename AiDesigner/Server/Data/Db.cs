...
namespace AiDesigner.Server.Data
{
    public class DBConnection
    {
        ...
        public async Task<Guid> SaveProgramAsync(ProgramObject programObject)
        {
            try
            {
                using (SqlConnection connection = new SqlConnection(_connectionString))
                {
                    ...
                    var query = @"
                        INSERT INTO nestor.programs (Id, Name, Description, Author, AuthorName, ApiKey, IsCustomBlock, IsPublic, ProgramData, SupportsSessions, Image, LastOpened )
        ...
        public async Task<bool> HasUserConnectionAsync(Guid programId, Guid userId)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var query = @"
                    SELECT COUNT(*)
                    FROM nestor.user_program_connections
        ...
        public async Task<IEnumerable<ProgramObject>> GetAllUserProgramsAsync(Guid userId)
        {
            ...
            FROM nestor.programs p
            JOIN nestor.user_program_connections upc ON p.Id = upc.ProgramId
        ...
        public async Task ConnectUserToProgramAsync(Guid userId, Guid programId)
        {
            ...
            {
                var query = @"
                    INSERT INTO nestor.user_program_connections (UserId, ProgramId, IsCustomBlock)
        ...
        public async Task DisconnectUserFromProgramAsync(Guid userId, Guid programId)
        {
            ...
            {
                var query = @"
                    DELETE FROM nestor.user_program_connections
        ...
        public async Task DeleteProgramAsync(Guid userId, Guid Id)
        {
            using (var connection = new SqlConnection(_connectionString))
            {
                var query = @"
                    ...
                    DELETE FROM nestor.programs
                    WHERE Id = @Id AND Author = @userId;

                    DELETE FROM nestor.Workshop_Article
                    WHERE ProgramId = @Id;

                    DELETE FROM nestor.User_Article
                    WHERE ArticleId IN (
                        SELECT Id
                        FROM nestor.Workshop_Article
                        WHERE ProgramId = @Id
                    );
                ";
        ...
        {
            StringBuilder query = new StringBuilder(@"SELECT COUNT(*) FROM nestor.Workshop_Article a WHERE 1=1 AND Status = 'Accepted'");
        ...
        public async Task<IEnumerable<WorkshopArticle>> SearchArticlesAsync(int start, int end, string searchTerm = null, string searchClass = null, string type = null)
        {
            StringBuilder query = new StringBuilder(@"
                ...
                FROM nestor.Workshop_Article a
                LEFT JOIN nestor.User_Article u ON a.Id = u.ArticleId
                LEFT JOIN nestor.programs p ON a.ProgramId = p.Id
    ...
    ");
        ...
        public async Task<int> GetCountOfTopRatedArticlesAsync()
        {
            const string query = @"
                ...
                FROM nestor.Workshop_Article a
                JOIN nestor.User_Article u ON a.Id = u.ArticleId
    ...
    ");
        ...
        {
            var query = @"
                INSERT INTO nestor.Workshop_Article (Id, Name, Description, SearchClass, AuthorId, AuthorName, ProgramId, ApiKey, ProgramImage, Type, Created, Downloads, Status)
        ...
        {
            var query = @"
                UPDATE nestor.Workshop_Article 
        ...
        {
            const string query = "DELETE FROM nestor.Workshop_Article WHERE Id = @Id;";
            await using SqlConnection connection = new SqlConnection(_connectionString);
        ...
        public async Task<IEnumerable<WorkshopArticle>> GetArticlesConnectedToUserAsync(string userId)
        {
            const string query = @"
                ...
                FROM nestor.Workshop_Article a
                INNER JOIN nestor.User_Article u ON a.Id = u.ArticleId
                LEFT JOIN nestor.User_Article ua ON a.Id = ua.ArticleId
                Left JOIN nestor.programs p ON a.ProgramId = p.Id
        ...
        public async Task<string> DeleteArticleImagesAsync(string articleId)
        {
            ...
            const string query = @"DELETE FROM nestor.Article_Images WHERE ArticleId = @ArticleId;";
        ...
        {
            var query = @"
        IF NOT EXISTS (SELECT 1 FROM nestor.User_Article WHERE UserId = @UserId AND ArticleId = @ArticleId)
        BEGIN
            INSERT INTO nestor.User_Article (UserId, ArticleId, IsCreator, Rating, Review, IsFavorite)
    ...
    ";
        ...
        {
            var query = @"
        UPDATE nestor.User_Article 
        SET Rating = @Rating, Review = @Review
        FROM nestor.User_Article 
        INNER JOIN nestor.Workshop_Article ON nestor.User_Article.ArticleId = nestor.Workshop_Article.Id
        WHERE nestor.User_Article.UserId = @UserId 
        AND nestor.Workshop_Article.ProgramId = @ProgramId;
    ";
    ...
    ";
        ...
        public async Task<IEnumerable<CustomProgram>> GetAllProgramsFromUserWorkshopArticlesAsync(string userId)
        {
            const string query = @"
                ...
                FROM programs p
                INNER JOIN nestor.Workshop_Article a ON p.Id = a.ProgramId
                INNER JOIN nestor.User_Article u ON a.Id = u.ArticleId
                WHERE u.UserId = @UserId
        ...
        {
            string query = @"SELECT TOP 4 * FROM nestor.News_Article 
        ...
        {
            string query = @"INSERT INTO nestor.News_Article (Id, Title, Content, PublishDate, ImageData)
            ...
            using (var connection = new SqlConnection(_connectionString))
            {
                var query = @"
                    ...
                    SELECT @TokensToAdd = CASE SubscriptionTier
                        ...
                        WHEN 2 THEN 8000
                        ELSE 500 
                    END FROM nestor.TokenWallet WHERE UserId = @UserId;

                    IF NOT EXISTS (SELECT 1 FROM nestor.TokenWallet WHERE UserId = @UserId)
                    BEGIN
                        INSERT INTO nestor.TokenWallet (Id, UserId, Tokens, LastRefill, BoughtTokens, SubscriptionTier)
                        OUTPUT inserted.Tokens, inserted.BoughtTokens
                        VALUES (NEWID(), @UserId, @TokensToAdd, @Today, 0, 0);
                    END
                    ELSE
                    BEGIN
                        UPDATE nestor.TokenWallet
...
WITH CombinedCalls AS (
    ...
    FROM
        nestor.Calls c
    INNER JOIN
        nestor.ApiKeys ak ON c.[Api/UserId] = ak.UserId
    WHERE
        ak.UserId = @UserId
)
SELECT TOP 100
...
ORDER BY
    ...
    ");
        ...
        public async Task<Guid> CreateSessionAsync(Session session)
        {
            ...
            {
                var query = @"
            INSERT INTO nestor.sessions (SessionId, UserId, ProgramId, Variables, SessionName, CreatedTime, LastEditedTime)
    ...
    ";
        ...
        {
            var insertQuery = @"
            IF NOT EXISTS (SELECT 1 FROM nestor.UserTutorials WHERE UserId = @UserId AND TutorialId = @TutorialId)
            BEGIN
                INSERT INTO nestor.UserTutorials (UserId, TutorialId, IsCompleted, CompletionDate)
                VALUES (@UserId, @TutorialId, 1, GETUTCDATE())
            END
            ELSE
            BEGIN
                UPDATE nestor.UserTutorials
        ...
        SELECT t.*
        FROM nestor.Tutorials t
        JOIN nestor.UserTutorials ut ON t.TutorialId = ut.TutorialId
    ...
    ";
        ...
        SELECT 
            ...
            ut.CompletionDate
        FROM nestor.Tutorials t
        LEFT JOIN nestor.UserTutorials ut ON t.TutorialId = ut.TutorialId AND ut.UserId = @UserId;
    ";
        ...
        {
            var query = @"
        INSERT INTO nestor.Tutorials (Name, Text, Image)
